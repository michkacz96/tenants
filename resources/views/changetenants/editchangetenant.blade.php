...
<div class="mt-3">
    <div class="row">
        ...
        <div class="col-md-12 col-lg-6">
            {!! Form::open(['action' => ['App\Http\Controllers\RentsController@updateRecord', $tenancy->id], 'method'
            => 'POST']) !!}
            <div class="row">
                <div class="form-group col-md-12 col-lg-6">
                    {{Form::label('start_date', 'Beginning of tenancy')}}
                    {{Form::date('start_date', $tenancy->start_rent_date, ['class' => 'form-control', 'required'])}}
                </div>
                <div class="form-group col-md-12 col-lg-6">
                    {{Form::label('end_date', 'End of tenancy')}}
                    {{Form::date('end_date', $tenancy->end_rent_date, ['class' => 'form-control'])}}
                </div>

                <div class="form-group col-md-12 col-lg-12">
                    {{Form::label('tenant', 'Tenant')}}
                    {{Form::select('tenant', $tenants, $tenancy->tenant_id, ['class' => 'form-select', 'required'])}}
                </div>
            </div>
            <div class="my-3 clearfix">
                {{Form::hidden('_method', 'PUT')}}
                {{Form::submit('Edit record', ['class' => 'btn btn-primary float-end'])}}
            </div>
            {!! Form::close() !!}
        </div>
    </div>
